...
- **交互式报告入口**：在 app.py 侧边栏添加「交互式报告」入口区域，包含描述文案和「打开演进报告」按钮，点击跳转到 `docs/index.html`
- **场景入口独立分页**：首页简化为仅展示标题和两个入口按钮（开始探索/查看演进报告），将 PvP/PvE/设计原则/检查清单/时间轴等场景入口卡片提取到独立的「设计场景」分页
- **演进报告独立分页**：在顶部导航栏和侧边栏新增「演进报告」入口，包含报告概览、设计原则、子场景、双轨对照四个子标签页
