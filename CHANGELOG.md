...
- **视觉增强**：场景详情页（PvP/PvE）历史教训区块增大标签字号（9px→13px）并添加分类色调（红色/绿色）；场景入口卡片增加顶部彩条、图标背景色和标签彩色化；各子章节分组标题增加左侧彩色竖条；数字统计（场景条目数/原则数/清单数）使用对应分类颜色
- **设计原则详情页布局重构**：将原本上下堆叠（列表在上、详情在下）的布局改为左右并排（列表在左、详情在右）；列表区域添加 sticky 定位和 max-height 限制，内容超长时左侧可独立滚动；选中项自动滚动至可视区域；详情区域添加独立滚动条
- **docs/index.html 还原至 v2.3**：移除周日新增的未归档文件，保留周五版本基础上叠加的交互式报告内容
