...
数据来源：https://xyq.163.com/{year}/mptz/ 及月度更新列表
目标：采集阵法系统的历史调整原始内容，填补 mhxy_history.py 中
     MHXY_FORMATION_EVOLUTION 只有 5 条记录、缺少原始文本的缺口。

输出文件：docs/mhxy_formation_history.json
"""
...
class MHXYFormationScraper:
    ...
    def _is_formation_page(self, text: str) -> bool:
        """判断页面内容是否与阵法相关"""
    ...
    def parse_patch_page(self, url: str) -> Optional[Dict[str, Any]]:
        """解析门派调整/维护公告详情页，返回阵法相关内容"""
        html = self.fetch_url(url)
        if not html:
            return None

        text = _extract_text(html)
        if not self._is_formation_page(text):
            return None
        ...
        result = {
            ...
            'content': text[:8000],
            'update_type': '阵法调整',
            'keywords': [kw for kw in FORMATION_KEYWORDS if kw in text],
        }
    ...
    def scrape_range(self, start_year: int = 2001, end_year: int = None) -> List[Dict[str, Any]]:
        """
        采集指定年份范围的阵法调整数据。

        阵法系统 2001 年上线，2005 年形成完整克制体系，
        此后每次大规模更新都可能涉及阵法调整。
        """
        if end_year is None:
        ...
        for year in range(start_year, end_year + 1):
            ...
            print(f"\n{'=' * 50}")
            print(f"采集 {year} 年阵法调整数据")
            print(f"{'=' * 50}")
            results = self.scrape_year(year)
            all_results.extend(results)
            print(f"  本年有效阵法记录: {len(results)} 条")
...
def main():
    ...
    # 阵法系统 2001 年上线，全量采集
    results = scraper.scrape_range(start_year=2001)
    ...
    output_file = Path(__file__).parent.parent / "docs" / "mhxy_formation_history.json"
    output_file.parent.mkdir(parents=True, exist_ok=True)
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print(f"\n采集完成，共 {len(results)} 条阵法相关记录")
